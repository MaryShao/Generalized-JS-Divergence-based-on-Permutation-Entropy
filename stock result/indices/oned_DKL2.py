...
def prob(fname, h): #simplified version
    point_l = fname
    N = len(point_l)
    L1=[]
    for i in range(N):
        point_l[i]=point_l[i]-min(point_l)+1/1000000000
    for i in range(N):
        L1.append(point_l[i]/(sum(point_l)+1/1000000))
    return L1
...
def prob_pentropy(fname,n):
    ...
    for num in standard_l.keys():
        standard_l[num] = standard_l[num] / (len(order_l)+1/1000000)
    return standard_l
